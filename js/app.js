...
let currentCategory = 'all'; // по умолчанию показываем все факты

// ========== ЗАЩИТА ОТ ПОВТОРОВ ==========
const shownFactsByCategory = {};

function getRandomFact(category) {
  // Собираем массив фактов
  let facts;
  if (category === 'all') {
    // Объединяем все категории в один плоский список
    facts = Object.values(categoriesData).flat();
  } else {
    facts = categoriesData[category];
  }

  // Инициализируем Set для хранения показанных индексов
  ...
  // Собираем индексы ещё не показанных фактов
...
// Кнопка "Получить случайный факт"
if (randomBtn) {
