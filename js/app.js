...
let currentCategory = 'all'; // все категории по умолчанию

// =================== ФУНКЦИИ ===================
function getRandomFact(category = 'all') {
  if (category === 'all') {
    // Собираем все факты в один массив
...
// =================== ОБРАБОТЧИКИ ===================
// Кнопка случайного факта (с учётом текущей категории)
if (randomBtn) {
...
actionBtns.forEach(btn => {
  btn.addEventListener('click', function() {
    if (!currentFact) {
      showToast("Сначала получите факт!", false);
      return;
    }
...
categoryBtns.forEach(btn => {
  btn.addEventListener('click', function() {
    ...
    if (categoriesData[categoryName]) {
      ...
      currentCategory = 'all'; // на случай, если текст не совпал
    }
    changeFact(); // сразу показываем новый факт из категории
  });
});

// =================== ЗАПУСК ===================
// Показываем первый факт (из всех категорий)
changeFact();
